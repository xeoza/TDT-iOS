...
class ChatLogController: UICollectionViewController {
	...
	func observeTypingIndicator() {
		...
		guard let conversationID = conversation?.chatID,
		...
		if let isGroupChat = conversation?.isGroupChat, isGroupChat {
			...
			typingIndicatorReference.observe(.value, with: { (snapshot) in
				...
				guard let dictionary = snapshot.value as? [String: AnyObject],
					let firstKey = dictionary.first?.key
				else {
			...
			typingIndicatorReference.observe(.value, with: { (isTyping) in
				guard let isParticipantTyping = isTyping.value! as? Bool,
					isParticipantTyping
				else {
					self.handleTypingIndicatorAppearance(isEnabled: false)
					return
	...
	func updateMessageStatus(messageRef: DatabaseReference) {
		...
		messageRef.child("fromId").observeSingleEvent(of: .value, with: { (snapshot) in
			...
			guard uid != senderID,
				self.navigationController?.visibleViewController is ChatLogController else {
				senderID = nil
				return
			}
	...
	func updateMessageStatusUI(sentMessage: Message) {
		...
		DispatchQueue.global(qos: .default).async {
			...
			guard let index = self.messages.firstIndex(where: { (message) -> Bool in
				message.messageUID == sentMessage.messageUID
			})
			else {
				return
			}
	...
	func updateMessageStatusUIAfterDeletion(sentMessage: Message) {
		...
		guard
			let uid = Auth.auth().currentUser?.uid, currentReachabilityStatus != .notReachable,
			let lastMessageUID = messages.last?.messageUID,
	...
	func startCollectionViewAtBottom() { // start chat log at bottom for iOS 10
		...
		guard Double(contentHeight) > Double(collectionHeight) else { return }
	...
	fileprivate func setupCollectionView() {
		...
		collectionView?.register(OutgoingPhotoMessageCell.self, forCellWithReuseIdentifier: photoMessageCellID)
	...
	func setupTitleName() {
		...
		if currentUserID == toId {
			navigationItem.setTitle(title: NameConstants.personalStorage, subtitle: "")
		} else {
			navigationItem.setTitle(title: conversation?.chatName ?? "", subtitle: "")
		}
	}
	...
	fileprivate func manageNavigationItemTitle(onlineStatusObject: AnyObject) -> String {
		...
		if let onlineStatusStringStamp = onlineStatusObject as? String {
			if onlineStatusStringStamp == statusOnline { // user online
				navigationItem.setTitle(title: title, subtitle: statusOnline)
				return statusOnline
...
//		}
	...
	var mediaPickerController: MediaPickerControllerNew!
	var voiceRecordingViewController: VoiceRecordingViewController!
