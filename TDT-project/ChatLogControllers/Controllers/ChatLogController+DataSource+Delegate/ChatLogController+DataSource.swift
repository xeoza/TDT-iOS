...
extension ChatLogController: UICollectionViewDelegateFlowLayout {
	...
	func selectSize(indexPath: IndexPath) -> CGSize {
		...
		guard indexPath.section == 0 else { return CGSize(width: collectionView!.frame.width, height: 40) }
		...
		guard !isInformationMessage else {
			guard let infoMessageWidth = collectionView?.frame.width, let messageText = message.text else {
