...
class VoiceRecordingViewController: UIViewController {
    ...
    @objc func updateAudioMeter(_: Timer) {
...
//    deleteAllRecordings()
    ...
    @objc func record(_: UIButton) {
        ...
        if recorder != nil, recorder.isRecording {
            ...
            //            recorder.record()
            recordWithPermission(false, completionHandler: { _ in })
        }
    }
...
//    }
    ...
    @objc func background(_: Notification) {
        print("\(#function)")
    }

    @objc func foreground(_: Notification) {
        print("\(#function)")
    }
...
extension VoiceRecordingViewController: AVAudioRecorderDelegate {
    ...
    func insertItemsToCollectionViewAnimated(at indexPath: [IndexPath], mediaObject _: [String: AnyObject]) {
        expandCollection()
        inputContainerView?.attachedImages.performBatchUpdates({
    ...
    func audioRecorderDidFinishRecording(_: AVAudioRecorder,
        ...
        do {
            ...
            if inputContainerView!.selectedMedia.count - 1 >= 0 {
                insertItemsToCollectionViewAnimated(at: [IndexPath(item: inputContainerView!.selectedMedia.count - 1, section: 0)], mediaObject: mediaObject)
            } else {
                insertItemsToCollectionViewAnimated(at: [IndexPath(item: 0, section: 0)], mediaObject: mediaObject)
            }
    ...
    func audioRecorderEncodeErrorDidOccur(_: AVAudioRecorder,
                                          error: Error?) {
        print("\(#function)")
